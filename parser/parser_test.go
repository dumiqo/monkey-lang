...
func TestPersingInfixExpressions(t *testing.T) {
	...
	for _, tt := range infixTests {
		...
		smtp, ok := program.Statements[0].(*ast.ExpressionStatement)
		if !ok {
			t.Fatalf("program.Statements[0] is not ast.ExpressionStatement. got=%T\n", program.Statements[0])
		}
		exp, ok := smtp.Expression.(*ast.InfixExpression)
		if !ok {
			t.Fatalf("smtp.Expression is not ast.InfixExpression. got=%T\n", smtp.Expression)
		}
